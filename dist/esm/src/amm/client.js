import { encodeFunctionData, hexToBigInt, bytesToHex, keccak256 } from 'viem';
import { conductorAbi } from '../execution/abis/conductor.js';
import { fetchNonce, fetchEip1559Fees, fetchAllowance } from '../execution/evm.js';
import { decodeSparkHumanReadableTokenIdentifier } from '../utils/tokenAddress.js';
import { getClient } from '../execution/rpc.js';
        ...
        : trimmed.startsWith("btkns")
            ...
            : trimmed.startsWith("btknt")
                ...
                : trimmed.startsWith("btknl")
                    ? "LOCAL"
                    : "MAINNET";
    const decoded = decodeSparkHumanReadableTokenIdentifier(trimmed, network);
    return {
        bech32m: trimmed,
...
class AMMClient {
    ...
    async ensureAllowance(tokenAddress, amountIn, timeoutMs = 30_000) {
        const execConfig = this.execClient.getConfig();
        const account = await this.execClient.getEvmAccount();
        const allowance = await fetchAllowance(execConfig.rpcUrl, tokenAddress, account.address, this.config.conductorAddress);
        ...
        while (Date.now() < deadline) {
            const current = await fetchAllowance(execConfig.rpcUrl, tokenAddress, account.address, this.config.conductorAddress);
            if (current >= amountIn)
                return;
