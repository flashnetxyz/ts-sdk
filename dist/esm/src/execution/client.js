...
import { sparkWalletToEvmAccount, getWalletSigner } from './spark-evm-account.js';
import { encodeWithdrawSats, encodeWithdrawToken } from './bridge.js';
import { fetchNonce, fetchEip1559Fees } from './evm.js';
import { resolveExpiresAt } from './types.js';
...
class ExecutionClient {
    ...
    async withdraw(params) {
        ...
        return this.submitIntent([], { type: "execute", signedTxHash: keccak256(signedTx) }, {
    ...
    async withdrawToken(params) {
        ...
        const account = await this.getEvmAccount();
        const sparkRecipient = await this.getSparkRecipientHex();
        const calldata = encodeWithdrawToken(params.tokenAddress, params.amount, sparkRecipient);
        const nonce = await fetchNonce(this.config.rpcUrl, account.address);
        const fees = await fetchEip1559Fees(this.config.rpcUrl);
        const signedTx = await account.signTransaction({
            to: this.config.bridgeAddress,
...
export { ExecutionClient, stringifyWithBigint };
