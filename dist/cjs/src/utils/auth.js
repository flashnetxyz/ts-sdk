        ...
        try {
            if (this.wallet) {
                ...
                const messageBytes = message.startsWith("0x")
                    ? hex.getUint8ArrayFromHex(message.slice(2))
                    : hex.getUint8ArrayFromHex(message);
                const messageHash = sha256__default.default(messageBytes);
                const signature = await this.signer.signMessage(messageHash);
                return hex.getHexFromUint8Array(signature);
            }
            else {
