...
function evmAddressFromCompressedPubkey(compressed) {
    const point = secp256k1.secp256k1.ProjectivePoint.fromHex(compressed);
    const uncompressed = point.toRawBytes(false); // 65 bytes: 04 + x + y
    const hash = viem.keccak256(viem.toHex(uncompressed.slice(1)));
...
async function signHash(signer, hash, expectedAddress) {
    ...
    const r = viem.toHex(compact.slice(0, 32));
    const s = viem.toHex(compact.slice(32, 64));
    const expectedLower = expectedAddress.toLowerCase();
    // Try yParity=0
    try {
        const recovered = await viem.recoverAddress({
            hash,
...
async function sparkWalletToEvmAccount(wallet) {
    ...
    const account = accounts.toAccount({
        ...
        async signMessage({ message }) {
            const hash = viem.hashMessage(message);
...
exports.sparkWalletToEvmAccount = sparkWalletToEvmAccount;
